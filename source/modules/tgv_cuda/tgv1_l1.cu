...
__global__ void tgv_kernel_part4_l1(
                  ...
                  Pixel alpha1)
{
    ...
    for(uint iteration_index = 0; iteration_index < iteration_count; iteration_index++)
    {
      ...
      */
        ...
        tgv_kernel_part4_l1<<<grid_dimension, block_dimension>>>(  p_x, p_y, p_z,
                                                                p_xx, p_yy, p_zz,
                                                                tau, u, f,
                                                                lambda,
...
template float* tgv1_l1_launch(float* f_host,
uint width, uint height, uint depth,
float lambda,
uint iteration_count,
uint paint_iteration_interval,
...
template double* tgv1_l1_launch(double* f_host,
uint width, uint height, uint depth,
double lambda,
uint iteration_count,
uint paint_iteration_interval,
