                      ...
                      Pixel* convergence_curve)
{
    ...
    for(uint iteration_index = 0; iteration_index < iteration_count; iteration_index++)
    {
        ...
        // convergence_curve ...
        convergence_curve[iteration_index] = 0;

        tgv_launch_forward_differences<Pixel>(u,
                p_x,p_y,p_z,
                width, height, depth,
                block_dimension,
                grid_dimension_x,
        ...
        printf("iteration: %d \n", iteration_index);
        for(int pixel_index = 0; pixel_index < voxel_count; pixel_index++)
        {
            Pixel diffx = (p_x[pixel_index] - v_x[pixel_index]) * mask_host[pixel_index];
            Pixel diffy = (p_y[pixel_index] - v_y[pixel_index]) * mask_host[pixel_index];
            if(depth > 1)
            {
            //    Pixel diffz = p_z[pixel_index] - v_z[pixel_index];
              //  convergence_curve[iteration_index] += std::sqrt(diffx*diffx +
                //                                                diffy*diffy +
                  //                                              diffz*diffz);
            }
            else
...
template float* tgvk_l1_deshade_convergence_launch(float* f_host,
                      const uint width, const uint height, const uint depth,
                      const float lambda,
                      const uint iteration_count,
                      const uint paint_iteration_interval,
                      ...
                      float* mask_host,
                      float* convergence_curve);

template double* tgvk_l1_deshade_convergence_launch(double* f_host,
                      const uint width, const uint height, const uint depth,
                      const double lambda,
                      const uint iteration_count,
                      const uint paint_iteration_interval,
